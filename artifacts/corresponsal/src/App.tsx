    ...
    <div className="paper-grid min-h-[calc(100dvh-72px)] overflow-hidden">
      <section className="mx-auto max-w-6xl px-5 pb-14 pt-12 md:px-10 md:pb-20 md:pt-20">
        <div className="max-w-3xl rise-in">
          ...
          <h1 className="font-display text-[clamp(2.1rem,8vw,7.3rem)] leading-[.9] tracking-[-0.065em] text-foreground">Quedate cerca<br /><span className="text-primary">desde acá.</span></h1>
...
function BriefHero({ brief }: { brief: PlaceBrief | undefined }) {
  ...
  return <article className="relative min-h-[340px] overflow-hidden rounded-3xl bg-sidebar p-7 text-sidebar-foreground md:p-10" data-testid="card-place-brief">{brief.imageUrl && <img src={brief.imageUrl} alt="" className="absolute inset-0 h-full w-full object-cover opacity-35 mix-blend-luminosity" />}{!brief.imageUrl && <div className="absolute -right-16 -top-20 h-72 w-72 rounded-full border-[38px] border-primary/25" />}<div className="relative flex h-full min-h-[276px] flex-col justify-between"><div className="flex items-center justify-between"><span className="rounded-full border border-sidebar-foreground/20 px-3 py-1 font-mono-ui text-[10px] uppercase tracking-[.14em] text-sidebar-foreground/70">El resumen</span><span className="font-mono-ui text-[10px] text-sidebar-foreground/50">{formatRelative(brief.updatedAt)}</span></div><div className="max-w-2xl"><p className="mb-3 flex items-center gap-2 text-sm text-sidebar-foreground/65"><Sun size={16} className="text-accent" />{brief.weather?.description ?? 'Un vistazo al día'}</p><h2 className="font-display text-[clamp(1.15rem,2.4vw,1.8rem)] leading-snug tracking-[-.01em]">{brief.summary || 'Acá está por comenzar un nuevo día.'}</h2></div></div></article>;
}

function TimeCard({ brief }: { brief: PlaceBrief | undefined }) {
  if (!brief) return null;
  return <aside className="flex min-h-[340px] flex-col justify-between rounded-3xl border border-border bg-card p-7 md:p-8" data-testid="card-local-time"><div className="flex items-center justify-between"><span className="font-mono-ui text-[10px] uppercase tracking-[.16em] text-muted-foreground">Ahora mismo</span><span className="h-2 w-2 rounded-full bg-[#5f9d85]" /></div><div><p className="font-mono-ui text-[clamp(1.8rem,6vw,4.7rem)] leading-none tracking-[-.08em] text-foreground">{formatTime(brief.localTime, brief.place.timezone)}</p><p className="mt-3 text-sm text-muted-foreground">hora local en {brief.place.displayName}</p></div><div className="flex items-end justify-between border-t hairline pt-4"><div><p className="font-mono-ui text-3xl tracking-[-.07em]">{Math.round(brief.weather?.temperature ?? 0)}°</p><p className="text-xs text-muted-foreground">clima actual</p></div><span className="rounded-xl bg-accent/50 px-3 py-2 text-xs text-foreground">{brief.weather?.description ?? 'Condiciones actuales'}</span></div></aside>;
}
