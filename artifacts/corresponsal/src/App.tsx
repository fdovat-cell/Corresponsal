...
function ExplorePage() {
  ...
  const [search, setSearch] = useState('');
  const nearbyParams = { lat: selected.lat, lon: selected.lon, category: category || undefined, ...(search ? { q: search } : {}) } as GetNearbyPlacesParams;
...
function NearbyPanel({ category, setCategory, search, setSearch, query }: { category: string; setCategory: (value: string) => void; search: string; setSearch: (value: string) => void; query: ReturnType<typeof useGetNearbyPlaces<NearbyPlace[]>> }) {
  ...
  return <aside className="rounded-2xl border border-border bg-card p-5 md:p-6" data-testid="panel-nearby"><div className="mb-5 flex items-start justify-between"><div><p className="font-mono-ui text-[10px] uppercase tracking-[.16em] text-primary">Cerca tuyo</p><h3 className="mt-2 font-display text-2xl tracking-[-.04em]">A la vuelta de la esquina</h3></div><MapPin size={19} className="text-primary" /></div><div className="relative mb-3"><Search size={14} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" /><input value={inputValue} onChange={(e) => setInputValue(e.target.value)} placeholder="Buscar un lugar por nombre..." className="w-full rounded-lg border border-border bg-background py-2 pl-8 pr-8 text-sm outline-none focus:border-primary/60" data-testid="input-nearby-search" />{inputValue && <button onClick={() => setInputValue('')} className="absolute right-2 top-1/2 -translate-y-1/2 rounded-md p-1 text-muted-foreground hover:bg-muted hover:text-foreground" aria-label="Limpiar búsqueda" data-testid="button-clear-nearby-search"><X size={14} /></button>}</div><div className={`mb-5 flex flex-wrap gap-1.5 ${search ? 'opacity-40' : ''}`}>{categories.map((item) => <button key={item.key} onClick={() => { setCategory(category === item.key ? '' : item.key); setInputValue(''); }} className={`rounded-full px-2.5 py-1 text-[11px] ${category === item.key && !search ? 'bg-secondary text-foreground' : 'bg-muted text-muted-foreground hover:text-foreground'}`} data-testid={`button-nearby-${item.key}`}>{item.label}</button>)}</div>{query.isLoading ? <div className="space-y-3"><div className="h-12 animate-pulse rounded-lg bg-muted" /><div className="h-12 animate-pulse rounded-lg bg-muted" /></div> : query.isError ? <p className="text-sm text-muted-foreground">Los lugares cercanos están tardando en cargar.</p> : !(query.data ?? []).length ? <div className="py-5 text-center"><Compass size={20} className="mx-auto mb-2 text-muted-foreground" /><p className="text-sm text-muted-foreground">No encontramos lugares cercanos.</p></div> : <div className="space-y-1">{(query.data ?? []).slice(0, 7).map((place, index) => <a href={place.mapUrl} target="_blank" rel="noreferrer" key={`${place.name}-${index}`} className="group block rounded-xl p-3 transition-colors hover:bg-muted" data-testid={`link-nearby-${index}`}><div className="flex items-center justify-between gap-2"><h4 className="text-sm font-medium group-hover:text-primary">{place.name}</h4><ExternalLink size={13} className="shrink-0 text-muted-foreground" /></div><p className="mt-1 text-xs text-muted-foreground">{categories.find((c) => c.key === place.category)?.label ?? place.category}{place.address ? ` · ${place.address}` : ''}</p><p className="mt-0.5 text-[11px] text-primary/80">Ver reseñas en Google Maps</p></a>)}</div>}</aside>;
}
