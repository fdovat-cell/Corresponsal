...
function formatTime(value?: string, timezone?: string | null) {
  if (!value) return '—';
  try {
    return new Intl.DateTimeFormat('en', { hour: 'numeric', minute: '2-digit', timeZone: timezone || undefined }).format(new Date(value));
  } catch {
    return value;
  }
}
...
function formatRelative(value?: string) {
  ...
  return `Updated ${Math.round(minutes / 1440)}d ago`;
}
    ...
    <div className="min-h-[100dvh] bg-background">
      <aside className={`fixed inset-y-0 left-0 z-40 flex w-[244px] flex-col bg-sidebar px-5 py-6 text-sidebar-foreground transition-transform duration-300 md:translate-x-0 ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="mb-11 flex items-center justify-between">
          ...
          <button className="rounded-md p-1 md:hidden" onClick={() => setMobileOpen(false)} aria-label="Close menu" data-testid="button-close-menu"><X size={18} /></button>
        </div>
      ...
      {mobileOpen && <button className="fixed inset-0 z-30 bg-sidebar/40 md:hidden" onClick={() => setMobileOpen(false)} aria-label="Close navigation" data-testid="button-overlay-menu" />}
      <main className="min-h-[100dvh] md:pl-[244px]">
        <header className="sticky top-0 z-20 flex h-[72px] items-center justify-between border-b hairline bg-background/90 px-5 backdrop-blur-md md:px-10">
          <button onClick={() => setMobileOpen(true)} className="rounded-lg p-2 hover:bg-muted md:hidden" aria-label="Open menu" data-testid="button-open-menu"><Menu size={20} /></button>
  ...
  return (
    <div className="relative z-10">
      <div className={`flex items-center gap-3 rounded-2xl border bg-card px-4 py-1 shadow-sm transition-all ${clean ? 'border-primary/60 shadow-[0_8px_28px_-18px_hsl(var(--primary))]' : 'border-border'}`}>
        ...
        {search && <button onClick={() => setSearch('')} className="rounded-md p-1 text-muted-foreground hover:bg-muted hover:text-foreground" aria-label="Clear search" data-testid="button-clear-search"><X size={16} /></button>}
    ...
    <div className="paper-grid min-h-[calc(100dvh-72px)] overflow-hidden">
      <section className="mx-auto max-w-6xl px-5 pb-14 pt-12 md:px-10 md:pb-20 md:pt-20">
        <div className="max-w-3xl rise-in">
          <div className="mb-6 flex items-center gap-2 font-mono-ui text-[10px] uppercase tracking-[0.2em] text-primary"><span className="h-px w-8 bg-primary" />The world, a little closer</div>
          <h1 className="font-display text-[clamp(3.2rem,8vw,7.3rem)] leading-[.9] tracking-[-0.065em] text-foreground">Stay close<br /><span className="text-primary">from here.</span></h1>
          <p className="mt-7 max-w-[470px] text-[17px] leading-relaxed text-muted-foreground md:text-lg">A quiet, current window into the cities where your people live — and the places you’re curious about.</p>
        </div>
      ...
      <footer className="mx-auto flex max-w-6xl flex-col gap-3 border-t hairline px-5 py-7 text-xs text-muted-foreground md:flex-row md:items-center md:justify-between md:px-10"><span className="font-display text-base text-foreground/70">corresponsal</span><span>Made for the in-between moments.</span></footer>
    </div>
  );
}
...
function PlaceCard({ place, index, selected, onSelect, onRemove }: { place: Place; index: number; selected: boolean; onSelect: (place: Place) => void; onRemove: (place: Place) => void }) {
  ...
  return (
    <div className={`group relative overflow-hidden rounded-2xl border p-5 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg ${selected ? 'border-primary/50 bg-card' : 'border-border bg-card/70'}`} data-testid={`card-place-${placeKey(place)}`}>
      ...
      <div className="relative z-10 mt-6 flex items-center justify-between border-t hairline pt-3 pointer-events-none"><span className="font-mono-ui text-[10px] uppercase tracking-[.1em] text-muted-foreground">{place.timezone?.split('/').pop()?.replace('_', ' ') ?? 'Local time'}</span><ChevronRight size={15} className="text-primary transition-transform group-hover:translate-x-1" /></div>
    </div>
  );
}

function AddPlaceCard({ onSearch }: { onSearch: () => void }) {
  return <button onClick={onSearch} className="flex min-h-[218px] flex-col items-center justify-center rounded-2xl border border-dashed border-border bg-card/30 text-muted-foreground transition-colors hover:border-primary/60 hover:bg-card/70" data-testid="button-add-place"><span className="mb-3 flex h-10 w-10 items-center justify-center rounded-full border border-border text-xl font-light">+</span><span className="text-sm">Add another place</span></button>;
}

function EmptyFavorites({ onSearch }: { onSearch: () => void }) {
  return <div className="flex min-h-[220px] flex-col items-center justify-center rounded-2xl border border-dashed border-border bg-card/40 px-6 text-center"><Bookmark size={21} className="mb-3 text-primary" /><h3 className="font-display text-xl">Your world is still taking shape</h3><p className="mt-1 max-w-sm text-sm text-muted-foreground">Search for a city above and keep the places you want to return to close.</p><button onClick={onSearch} className="mt-5 rounded-full bg-primary px-4 py-2 text-sm font-medium text-primary-foreground transition-transform hover:scale-[1.03]" data-testid="button-search-first-place">Find a place</button></div>;
}
...
function PlaceLayout({ children }: { children: ReactNode }) {
  ...
  if (!selected) return <div className="mx-auto max-w-3xl px-5 py-24 text-center"><Compass className="mx-auto mb-4 text-primary" /><h1 className="font-display text-4xl">Choose a place first</h1><p className="mt-3 text-muted-foreground">Your daily window is waiting for a city.</p><Link href="/" className="mt-7 inline-flex rounded-full bg-primary px-5 py-2.5 text-sm font-medium text-primary-foreground" data-testid="link-choose-place">Choose a place</Link></div>;
  return <div className="mx-auto max-w-6xl px-5 py-10 md:px-10 md:py-14"><div className="mb-10 flex flex-wrap items-end justify-between gap-5"><div><Link href="/" className="mb-4 inline-flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground" data-testid="link-back-places"><ChevronRight size={13} className="rotate-180" />All places</Link><h1 className="font-display text-[clamp(2.8rem,6vw,5rem)] leading-none tracking-[-0.06em]">{selected.displayName}</h1><p className="mt-3 flex items-center gap-2 text-sm text-muted-foreground"><MapPin size={14} className="text-primary" />{selected.country}{selected.state ? `, ${selected.state}` : ''}<span className="text-border">/</span><Clock3 size={13} />{selected.timezone?.replace('_', ' ') ?? 'Local time'}</p></div><div className="flex rounded-xl border border-border bg-card p-1"><Link href="/place" className={`rounded-lg px-3.5 py-2 text-sm ${location === '/place' ? 'bg-secondary font-medium text-foreground' : 'text-muted-foreground hover:text-foreground'}`} data-testid="link-tab-today">Today</Link><Link href="/place/explore" className={`rounded-lg px-3.5 py-2 text-sm ${location === '/place/explore' ? 'bg-secondary font-medium text-foreground' : 'text-muted-foreground hover:text-foreground'}`} data-testid="link-tab-explore">Explore</Link></div></div>{children}</div>;
}
...
function PlacePage() {
  ...
  return <PlaceLayout>{briefQuery.isLoading ? <LoadingBrief /> : briefQuery.isError ? <ErrorPanel onRetry={() => briefQuery.refetch()} /> : <><div className="grid gap-4 md:grid-cols-[1.35fr_.65fr]"><BriefHero brief={brief} /><TimeCard brief={brief} /></div><section className="mt-14"><SectionHeading eyebrow="From there, today" title="The local read" action={<Link href="/place/explore" className="flex items-center gap-1 text-sm text-primary hover:gap-2 transition-all" data-testid="link-explore-more">Explore more <ArrowUpRight size={15} /></Link>} />{newsQuery.isLoading ? <NewsSkeleton /> : newsQuery.isError ? <ErrorPanel onRetry={() => newsQuery.refetch()} compact /> : <NewsList news={news} />}</section></>}</PlaceLayout>;
}

function BriefHero({ brief }: { brief: PlaceBrief | undefined }) {
  if (!brief) return null;
  return <article className="relative min-h-[340px] overflow-hidden rounded-3xl bg-sidebar p-7 text-sidebar-foreground md:p-10" data-testid="card-place-brief">{brief.imageUrl && <img src={brief.imageUrl} alt="" className="absolute inset-0 h-full w-full object-cover opacity-35 mix-blend-luminosity" />}{!brief.imageUrl && <div className="absolute -right-16 -top-20 h-72 w-72 rounded-full border-[38px] border-primary/25" />}<div className="relative flex h-full min-h-[276px] flex-col justify-between"><div className="flex items-center justify-between"><span className="rounded-full border border-sidebar-foreground/20 px-3 py-1 font-mono-ui text-[10px] uppercase tracking-[.14em] text-sidebar-foreground/70">The brief</span><span className="font-mono-ui text-[10px] text-sidebar-foreground/50">{formatRelative(brief.updatedAt)}</span></div><div className="max-w-2xl"><p className="mb-3 flex items-center gap-2 text-sm text-sidebar-foreground/65"><Sun size={16} className="text-accent" />{brief.weather?.description ?? 'A view into the day'}</p><h2 className="font-display text-[clamp(2rem,4vw,3.6rem)] leading-[1.02] tracking-[-.05em]">{brief.summary || 'A new day is unfolding here.'}</h2></div></div></article>;
}

function TimeCard({ brief }: { brief: PlaceBrief | undefined }) {
  if (!brief) return null;
  return <aside className="flex min-h-[340px] flex-col justify-between rounded-3xl border border-border bg-card p-7 md:p-8" data-testid="card-local-time"><div className="flex items-center justify-between"><span className="font-mono-ui text-[10px] uppercase tracking-[.16em] text-muted-foreground">Right now</span><span className="h-2 w-2 rounded-full bg-[#5f9d85]" /></div><div><p className="font-mono-ui text-[clamp(2.7rem,6vw,4.7rem)] leading-none tracking-[-.08em] text-foreground">{formatTime(brief.localTime, brief.place.timezone)}</p><p className="mt-3 text-sm text-muted-foreground">local time in {brief.place.displayName}</p></div><div className="flex items-end justify-between border-t hairline pt-4"><div><p className="font-mono-ui text-3xl tracking-[-.07em]">{Math.round(brief.weather?.temperature ?? 0)}°</p><p className="text-xs text-muted-foreground">current weather</p></div><span className="rounded-xl bg-accent/50 px-3 py-2 text-xs text-foreground">{brief.weather?.description ?? 'Current conditions'}</span></div></aside>;
}
...
function EmptyFeed() {
  return <div className="rounded-2xl border border-dashed border-border bg-card/50 px-6 py-12 text-center"><Newspaper size={21} className="mx-auto mb-3 text-primary" /><h3 className="font-display text-xl">A quiet news day</h3><p className="mt-1 text-sm text-muted-foreground">There’s nothing new to bring you right now. Check back later.</p></div>;
}

function ErrorPanel({ onRetry, compact = false }: { onRetry: () => void; compact?: boolean }) {
  return <div className={`rounded-2xl border border-destructive/20 bg-destructive/5 text-center ${compact ? 'px-6 py-10' : 'px-6 py-20'}`}><p className="font-display text-xl">This window is briefly out of reach</p><p className="mt-1 text-sm text-muted-foreground">We couldn’t refresh this place just now.</p><button onClick={onRetry} className="mt-4 rounded-full border border-border bg-card px-4 py-2 text-sm font-medium hover:bg-muted" data-testid="button-retry">Try again</button></div>;
}
...
function ExplorePage() {
  ...
  const topics = ['Culture', 'Business', 'Politics', 'Community'];
  return <PlaceLayout><div className="mb-10 max-w-2xl"><p className="font-mono-ui text-[10px] uppercase tracking-[.18em] text-primary">A wider lens</p><h2 className="mt-3 font-display text-[clamp(2.5rem,5vw,4.8rem)] leading-[.94] tracking-[-.06em]">Find your way<br />around {selected.displayName}.</h2><p className="mt-5 text-base leading-relaxed text-muted-foreground">Follow a thread, then wander a little. Explore what’s being talked about and what’s nearby.</p></div><div className="mb-14 flex flex-wrap gap-2 border-b hairline pb-6">{topics.map((item) => <button onClick={() => setTopic(topic === item ? '' : item)} key={item} className={`rounded-full border px-4 py-2 text-sm transition-colors ${topic === item ? 'border-primary bg-primary text-primary-foreground' : 'border-border bg-card text-muted-foreground hover:border-primary/50 hover:text-foreground'}`} data-testid={`button-topic-${item.toLowerCase()}`}>{item}</button>)}</div><section><SectionHeading eyebrow="Keep reading" title={topic ? `${topic} in the news` : 'What’s moving there'} /><div className="grid gap-10 lg:grid-cols-[1fr_330px]"><div>{newsQuery.isLoading ? <NewsSkeleton /> : newsQuery.isError ? <ErrorPanel onRetry={() => newsQuery.refetch()} compact /> : <NewsList news={newsQuery.data ?? []} />}</div><NearbyPanel category={category} setCategory={setCategory} query={nearbyQuery} /></div></section></PlaceLayout>;
}
...
function NearbyPanel({ category, setCategory, query }: { category: string; setCategory: (value: string) => void; query: ReturnType<typeof useGetNearbyPlaces<NearbyPlace[]>> }) {
  const categories = ['Places to eat', 'Museums', 'Parks', 'Markets'];
  return <aside className="rounded-2xl border border-border bg-card p-5 md:p-6" data-testid="panel-nearby"><div className="mb-5 flex items-start justify-between"><div><p className="font-mono-ui text-[10px] uppercase tracking-[.16em] text-primary">Nearby</p><h3 className="mt-2 font-display text-2xl tracking-[-.04em]">A few doors down</h3></div><MapPin size={19} className="text-primary" /></div><div className="mb-5 flex flex-wrap gap-1.5">{categories.map((item) => <button key={item} onClick={() => setCategory(category === item ? '' : item)} className={`rounded-full px-2.5 py-1 text-[11px] ${category === item ? 'bg-secondary text-foreground' : 'bg-muted text-muted-foreground hover:text-foreground'}`} data-testid={`button-nearby-${item.toLowerCase().replaceAll(' ', '-')}`}>{item}</button>)}</div>{query.isLoading ? <div className="space-y-3"><div className="h-12 animate-pulse rounded-lg bg-muted" /><div className="h-12 animate-pulse rounded-lg bg-muted" /></div> : query.isError ? <p className="text-sm text-muted-foreground">Nearby places are taking a moment to load.</p> : !(query.data ?? []).length ? <div className="py-5 text-center"><Compass size={20} className="mx-auto mb-2 text-muted-foreground" /><p className="text-sm text-muted-foreground">No nearby places found.</p></div> : <div className="space-y-1">{(query.data ?? []).slice(0, 7).map((place, index) => <a href={place.mapUrl} target="_blank" rel="noreferrer" key={`${place.name}-${index}`} className="group block rounded-xl p-3 transition-colors hover:bg-muted" data-testid={`link-nearby-${index}`}><div className="flex items-center justify-between gap-2"><h4 className="text-sm font-medium group-hover:text-primary">{place.name}</h4><ExternalLink size={13} className="shrink-0 text-muted-foreground" /></div><p className="mt-1 text-xs text-muted-foreground">{place.category}{place.address ? ` · ${place.address}` : ''}</p></a>)}</div>}</aside>;
}
